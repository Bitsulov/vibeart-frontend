...
import { deleteUserErrorHandler } from "./deleteUserErrorHandler";
...
describe("deleteUserErrorHandler - показывает уведомление об ошибке удаления пользователя", () => {
    it("Показывает уведомление о ненайденном пользователе (404)", () => {
        const dispatch = vi.fn();

        deleteUserErrorHandler(createError(404), dispatch);
    ...
    it("Показывает уведомление о запрете (403)", () => {
        ...
        deleteUserErrorHandler(createError(403), dispatch);
    ...
    it("Показывает уведомление об ошибке сервера (500)", () => {
        ...
        deleteUserErrorHandler(createError(500), dispatch);
    ...
    it("Показывает уведомление о неизвестной ошибке при отсутствии тела ответа", () => {
        ...
        deleteUserErrorHandler(createError(), dispatch);

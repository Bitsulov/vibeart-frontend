...
import { postRequestErrorHandler } from "./postRequestErrorHandler";
...
describe("postRequestErrorHandler - показывает уведомление об ошибке создания или обновления поста", () => {
    it("Показывает уведомление о запрете (403)", () => {
        const dispatch = vi.fn();

        postRequestErrorHandler(createError(403), dispatch);
    ...
    it("Показывает уведомление об ошибке сервера (500)", () => {
        ...
        postRequestErrorHandler(createError(500), dispatch);
    ...
    it("Показывает уведомление о неизвестной ошибке при отсутствии тела ответа", () => {
        ...
        postRequestErrorHandler(createError(), dispatch);

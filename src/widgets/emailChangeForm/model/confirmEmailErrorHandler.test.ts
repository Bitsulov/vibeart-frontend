...
import { confirmEmailErrorHandler } from "./confirmEmailErrorHandler";
...
describe("confirmEmailErrorHandler - показывает уведомление об ошибке подтверждения изменения email", () => {
    it("Показывает уведомление о ненайденном пользователе (404)", () => {
        const dispatch = vi.fn();

        confirmEmailErrorHandler(createError(404), dispatch);
    ...
    it("Показывает уведомление об истёкшем коде (410)", () => {
        ...
        confirmEmailErrorHandler(createError(410), dispatch);
    ...
    it("Показывает уведомление о неверном коде (400)", () => {
        ...
        confirmEmailErrorHandler(createError(400), dispatch);
    ...
    it("Показывает уведомление о запрете (403)", () => {
        ...
        confirmEmailErrorHandler(createError(403), dispatch);
    ...
    it("Показывает уведомление об ошибке сервера (500)", () => {
        ...
        confirmEmailErrorHandler(createError(500), dispatch);
    ...
    it("Показывает уведомление о неизвестной ошибке при отсутствии тела ответа", () => {
        ...
        confirmEmailErrorHandler(createError(), dispatch);

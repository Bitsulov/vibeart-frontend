...
describe("reportErrorHandler - показывает уведомление об ошибке отправки жалобы", () => {
    it("Показывает уведомление о запрете (403)", () => {
        const dispatch = vi.fn();

        reportErrorHandler(createError(403), dispatch);
    ...
    it("Показывает уведомление об ошибке сервера (500)", () => {
        ...
        reportErrorHandler(createError(500), dispatch);
    ...
    it("Показывает уведомление о неизвестной ошибке при отсутствии тела ответа", () => {
        ...
        reportErrorHandler(createError(), dispatch);
